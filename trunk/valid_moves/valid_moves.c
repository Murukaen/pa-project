...
UCHAR VM_is_Check_if_piece_moves(STATE st, P_LOC loc, P_LOC who_check) {
	...
	List l = ST_get_List_Table_Location(st, col_on_move, T_K);
	P_LOC king_loc = first_nod_list(&l);
	UCHAR row_king = LOC_get_row(king_loc), col_king = LOC_get_col(king_loc), row_piece = LOC_get_row(loc), col_piece = LOC_get_col(loc), tag;

	/*fac bitmapul cu piesa la mutare, si bitmapul care arata locurile ce ataca regele*/
	BITMAP piece_pos = BM_Make_coord(row_piece, col_piece), attack_to_king = Moves[T_Q - 2][row_king][col_king];
	...
	if ((piece_pos & attack_to_king) == 0) {// daca locatia piesei n are treaba cu cea a regelui
		...
		if (row_king == row_piece) { //daca e pe aceeasi linie(COLOANA)
			...
			/* LOG */
			log_print("problema la validare mutare , verificare sah :( cam o ora debugg estimez!!", LOG_VALIDATE_MOVE_FILE);
			/* END LOG */
	...
	/* LOG */
	log_print("validare mutare sah, s a ajuns unde nu trebuia , e nashpa!", LOG_VALIDATE_MOVE_FILE);
	/* END LOG */
...
BITMAP VM_get_valid_if_check(STATE st, P_LOC pinned_piece, P_LOC checking_piece) {
	...
	UCHAR row_checking_piece = LOC_get_row(checking_piece), col_checking_piece = LOC_get_col(checking_piece), col_on_move =
			ST_get_col_on_move(st), row_pinned_piece = LOC_get_row(pinned_piece), col_pinned_piece = LOC_get_col(pinned_piece), N = T_N,
			P = T_P, Q = T_Q, R = T_R, B = T_B;
		...
		if (col_on_move == 0) {// daca e pion alb pinuit
			if (row_king > row_pinned_piece && col_king < col_pinned_piece) {// daca regele e in dreapta jos
				if ((row_pinned_piece - 1 == row_checking_piece)//si gasesc nebun sau regina neagra in stanga sus
					...
					rezultat = BM_Make_coord(row_pinned_piece - 1, col_pinned_piece + 1);
					return rezultat;
				}
			}

			if (row_king < row_pinned_piece && col_king < col_pinned_piece) {// daca regele e in stanga jos
				if ((row_pinned_piece + 1 == row_checking_piece) && (col_pinned_piece + 1 == col_checking_piece)) {// daca pionu alb are piesa care l pune in pin in dreapta sus
			...
			if (row_king > row_pinned_piece && col_king > col_pinned_piece) {// daca regele e in dreapta sus
				if ((row_pinned_piece - 1 == row_checking_piece) && (col_pinned_piece - 1 == col_pinned_piece)) { // daca piesa ce l pune in pin e in stanga jos
					rezultat = BM_Make_coord(row_pinned_piece - 1, col_pinned_piece - 1);
					return rezultat;
				}
			}
			if (row_king < row_pinned_piece && col_king > col_pinned_piece) {// daca rege in stanga sus
				if ((row_pinned_piece + 1 == row_checking_piece) && (col_pinned_piece - 1 == col_pinned_piece)) {// daca piesa ce l pune in pin e in dreapta jos
					rezultat = BM_Make_coord(row_pinned_piece + 1, col_pinned_piece - 1);
					return rezultat;
				}
			}
	...
	if (ST_get_tag_Table_What(st, row_pinned_piece, col_pinned_piece) == Q) {
		...
		if (ST_get_tag_Table_What(st, row_checking_piece, col_checking_piece) == Q) {
			// daca reginele sunt conectate pe linie
			if (row_checking_piece == row_pinned_piece || col_checking_piece == col_pinned_piece) {
		...
		if (ST_get_tag_Table_What(st, row_checking_piece, col_checking_piece) == R) {
			pt_regina = 1;
		}
		if (ST_get_tag_Table_What(st, row_checking_piece, col_checking_piece) == B) {
			pt_regina = 2;
		}
	}
	if (ST_get_tag_Table_What(st, row_pinned_piece, col_pinned_piece) == R || pt_regina == 1) {
	...
	if (ST_get_tag_Table_What(st, row_pinned_piece, col_pinned_piece) == B || pt_regina == 2) {

		// daca piesa ce da sah e pe diag dreapta sus
		if (row_checking_piece > row_pinned_piece && col_checking_piece > col_pinned_piece) {

			for (i = row_pinned_piece + 1, j = col_pinned_piece + 1; i <= row_checking_piece && j <= col_checking_piece; i++, j++) {
...
int VM_validate_King_move(STATE st, LOC loc) {
	...
	UCHAR row = loc.row, col = loc.col, col_on_move = ST_get_col_on_move(st), tag, regina, pion, tura, nebun, rege;
	LOC Locuri[8];
	int i, j;
	...
	if (col_on_move == 0) {
		for (i = 0; i < 8; i++) {
			if (Locuri[i].row >= 0 && Locuri[i].col < 8 && Locuri[i].row < 8 && Locuri[i].col >= 0)
				if (ST_get_tag_Table_What(st, row - 1, col + 1) == pion || ST_get_tag_Table_What(st, row + 1, col + 1) == pion) {// daca sunt pioni in fata stanga/dreapta
					return 1;
...
BITMAP VM_valid_moves(STATE st, P_LOC loc_piesa) {
	...
	UCHAR piece_row = LOC_get_row(loc_piesa), piece_col = LOC_get_col(loc_piesa);
	int i, j, regina = 0;
	BITMAP rezultat = 0;
	UCHAR ce_piesa, col_on_move = ST_get_col_on_move(st), captura_stanga, captura_dreapta, K = T_K, B = T_B, Q = T_Q, R = T_R, P = T_P, N = T_N;
	...
	if (ce_piesa_e_la_loc_piesa == N) {
		...
		rezultat = VM_valid_pos(Moves[T_N - 2][piece_row][piece_col], ST_get_bitmap(st, col_on_move));
		...
		if (col_on_move == 0) {// daca pioni albi
			...
			if (piece_col == 1) {// daca pioni pe pozitia de start
				...
				if (ST_get_tag_Table_What(st, piece_row, piece_col + 1) == T_NA) {// daca e liber pe prima pozitie
					...
					if (ST_get_tag_Table_What(st, piece_row, piece_col + 2) == T_NA) {// daca e liber pe a 2 a pozitie
						BM_Put_piece_at_coord(&rezultat, piece_row, piece_col + 2);
					}
				}
			...
			captura_stanga = ST_get_tag_Table_What(st, piece_row - 1, piece_col + 1);
			captura_dreapta = ST_get_tag_Table_What(st, piece_row + 1, piece_col + 1);
	...
	if (ce_piesa_e_la_loc_piesa == K) {
		...
		for (i = 0; i < 8; i++) {
			...
			if (VM_validate_King_move(st, Locuri[i]) == 0) {
				if (Locuri[i].row >= 0 && Locuri[i].row < 8 && Locuri[i].col >= 0 && Locuri[i].col < 8) {
					BM_Put_piece_at_coord(&rezultat, Locuri[i].row, Locuri[i].col);
				}
			}
		}
